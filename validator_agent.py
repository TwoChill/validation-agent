#!/usr/bin/env python3
"""
Validation Agent — Two-Speed Design
─────────────────────────────────────────────────────────────────────────────
...
STRATEGY 2 — Full Project (manual):
  python validator_agent.py --project /path/to/project [--full]
...
def run_per_file_mode(file_path: Path, log_path: Path) -> int:
    ...
    skip_names = {"validator_agent.py", "conftest.py", "setup.py"}
...
def run_full_mode(project_dir: Path, log_path: Path) -> int:
    ...
    print(f"\n[Validation Agent — Full Mode] {project_name}")
...
def main():
    ...
    if not has_project:
        ...
        parser = argparse.ArgumentParser(
            description="Validation Agent — Full Project Mode\n"
                        "Usage: python validator_agent.py --project /path [--full] [--log path]"
        )
